...
public class DiscoveryServerProcessor implements Runnable{
	 ...
	 * @param database
	 */
	public DiscoveryServerProcessor(Socket clientSocket, BackEndServerDatabase database) {
	...
	@Override
	public void run() {
		System.out.println("DiscoveryServerProcessor started");
		HTTPServer.log.debug("DiscoveryServerProcessor  started");
		...
		try {
			...
			System.out.println("DiscoveryServerProcessor IOException failed to create BufferedReader\nError Message: " + e.getLocalizedMessage());
			HTTPServer.log.debug("DiscoveryServerProcessor IOException failed to create BufferedReader\nError Message: " + e.getLocalizedMessage());
			return;
		} 
		...
		try {
			...
			System.out.println("DiscoveryServerProcessor IOException failed to create OutputStream\nError Message: " + e.getLocalizedMessage());
			HTTPServer.log.debug("DiscoveryServerProcessor IOException failed to create OutputStream\nError Message: " + e.getLocalizedMessage());
			return;
		}
		...
		try {
			while ((line = in.readLine()) != null) {
				...
				System.out.println("DiscoveryServerProcessor RECEIVED" + line);
				HTTPServer.log.debug("DiscoveryServerProcessor RECEIVED" + line);
				 ...
				 */
				if (line.equals("") ) {
					System.out.println("DiscoveryServerProcessor STOP READING");
					HTTPServer.log.debug("DiscoveryServerProcessor STOP READING");
					break; 
				}
			...
			System.out.println("DiscoveryServerProcessor IOException failed to read request.\nError Message: " + e.getLocalizedMessage());
			HTTPServer.log.debug("DiscoveryServerProcessor IOException failed to read request.\nError Message: " + e.getLocalizedMessage());
			return;
		} 		
		...
		if (requestLine != null){
				...
				if (body.length() <= 0){
					...
					if (correctlyFormatted){
						...
						if (uri.equals("/discovery")){
							...
							String PORT = (String) requestLine.getValueFromParam("PORT");
							...
							if (IP != null && PORT != null){
								...
								System.out.println("PRIMARY SERVER UPDATED");
								System.out.println("IP: " + IP + " PORT: " + PORT);
								
								database.setPrimaryInfo(IP, Integer.parseInt(PORT));
			 ...
			 */
				...
				if (uri.equals("/discovery")){		
					...
					json2.put("IP", database.getPrimaryIP());
					json2.put("Port", database.getPrimaryPort());
	...
	public void forwardToSecondaries(ArrayList<String> hashtags, String tweet){
